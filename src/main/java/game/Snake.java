...
public class Snake extends JPanel implements Runnable {
	...
	private static final String SNAKE_STRING = "SNAKE";
	...
	int[][] grid;
	transient List<Point> snake;
	transient List<Point> treats;
	Font smallFont;
...
//	}
	...
	void addTreat() {
		if (treats.size() < 10 && rand.nextInt(10) == 0) {
			if (rand.nextInt(4) != 0) {
				int x;
				int y;
				while (true) {
	...
	void drawStartScreen(Graphics2D g) {
		g.setColor(Color.red);
		g.setFont(getFont());
		g.drawString(SNAKE_STRING, 240, 190);
		g.setColor(Color.orange);
		g.setFont(smallFont);
		g.drawString("(Cliquer pour jouer)", 250, 240);
	}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> {
			...
			mainFrame.setTitle(SNAKE_STRING);
